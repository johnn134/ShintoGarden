...
public class Branch4D : MonoBehaviour {
	...
	void OnMouseDown() {
		if(canSnip) {
			transform.parent.GetComponent<Branch4D>().registerBranchRemoved();
			manager.GetComponent<BonsaiManager4D>().removeBranch();
	...
	bool processBranches() {
		...
		if(temp.name.Length >= 6) {
			if(temp.name.Substring(0, 6) == "Branch") {
				temp.GetComponent<Branch4D>().processGrowthCycle(branch, leaf, bud);
			}
		}
	...
	bool processBuds() {
		...
		if(temp.name.Length >= 3) {
			if(temp.name.Substring(0, 3) == "Bud") {
				temp.GetComponent<Bud4D>().processGrowthCycle(branch, leaf, bud);
			}
		}
	...
	bool processLeaves() {
		...
		if(temp.name.Length >= 4) {
			if(temp.name.Substring(0, 4) == "Leaf") {
				temp.GetComponent<Leaf4D>().processGrowthCycle(branch, leaf, bud);
			}
		}
	...
	void addBud(GameObject bud, Vector3 pos, Quaternion rot, bool isLeaf) {
		...
		newBud.transform.GetComponent<Bud4D>().setisLeaf(isLeaf);
		newBud.transform.GetComponent<Bud4D>().setDepth(depth + 1);
		newBud.transform.GetComponent<Bud4D>().setWPosition(Mathf.Clamp(w + Random.Range(-1, 2), 0, 6));   //the w value is clamped between 0 and 6 inclusive
		newBud.transform.GetComponent<Bud4D>().setManager(manager);
	}
	...
	bool checkIfAllLeavesAreDead() {
		...
		for(int i = transform.childCount - 1; i > 2; i--) {
			if(transform.GetChild(i).name.Length >= 4) {
				if(transform.GetChild(i).name.Substring(0, 4) == "Leaf") {
					if(transform.GetChild(i).GetComponent<Leaf4D>().getIsAlive()) {
						allDead = false;
					}
				}
			}
	...
	bool checkIfParentIsDiseased() {
		if(depth > 0) {
			Branch4D bm = transform.parent.GetComponent<Branch4D>();
			if(bm.hasDisease) {
	...
	void spreadDisease() {
		...
		for(int i = transform.childCount - 1; i > 2; i--) {
			...
			if(transform.GetChild(i).name.Length >= 4) {
				if(transform.GetChild(i).name.Substring(0, 4) == "Leaf") {
					transform.GetChild(i).GetComponent<Leaf4D>().setIsAlive(false);
				}
			}
		...
		if(depth > 1)
			transform.parent.GetComponent<Branch4D>().setHasDisease(true);
	}
	...
	public void setWPosition(int newW) {
		...
		this.transform.GetChild(0).GetChild(0).GetComponent<HyperObject>().updateMaterialShaderValues(newW);
		this.transform.GetChild(0).GetChild(1).GetComponent<HyperObject>().updateMaterialShaderValues(newW);
		this.transform.GetChild(0).GetChild(2).GetComponent<HyperObject>().updateMaterialShaderValues(newW);
	}
