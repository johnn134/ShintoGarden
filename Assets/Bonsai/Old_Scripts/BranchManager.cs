...
public class BranchManager : MonoBehaviour {
	...
	void OnMouseDown() {
		if(canSnip) {
			transform.parent.GetComponent<BranchManager>().registerBranchRemoved();
	...
	bool processBranches() {
		...
		if(temp.name.Length >= 6) {
			if(temp.name.Substring(0, 6) == "Branch") {
				temp.GetComponent<BranchManager>().processGrowthCycle(branch, leaf, bud);
			}
		}
	...
	bool processBuds() {
		...
		if(temp.name.Length >= 3) {
			if(temp.name.Substring(0, 3) == "Bud") {
				temp.GetComponent<BudManager>().processGrowthCycle(branch, leaf, bud);
			}
		}
	...
	bool processLeaves() {
		...
		if(temp.name.Length >= 4) {
			if(temp.name.Substring(0, 4) == "Leaf") {
				temp.GetComponent<LeafManager>().processGrowthCycle(branch, leaf, bud);
			}
		}
	...
	void addBud(GameObject bud, Vector3 pos, Quaternion rot, bool isLeaf) {
		...
		newBud.transform.GetComponent<BudManager>().setisLeaf(isLeaf);
		newBud.transform.GetComponent<BudManager>().setDepth(depth + 1);
		newBud.transform.GetComponent<BudManager>().setWPosition(Mathf.Clamp(w + Random.Range(-1, 2), 0, 6));   //the w value is clamped between 0 and 6 inclusive
		newBud.transform.GetComponent<BudManager>().setManager(manager);
	}
	...
	bool checkIfAllLeavesAreDead() {
		...
		for(int i = transform.childCount - 1; i > 2; i--) {
			if(transform.GetChild(i).name.Length >= 4) {
				if(transform.GetChild(i).name.Substring(0, 4) == "Leaf") {
					if(transform.GetChild(i).GetComponent<LeafManager>().getIsAlive()) {
						allDead = false;
					}
				}
			}
	...
	bool checkIfParentIsDiseased() {
		if(depth > 0) {
			BranchManager bm = transform.parent.GetComponent<BranchManager>();
			if(bm.hasDisease) {
	...
	void spreadDisease() {
		...
		for(int i = transform.childCount - 1; i > 2; i--) {
			...
			if(transform.GetChild(i).name.Length >= 6) {
				if(transform.GetChild(i).name.Substring(0, 6) == "Branch") {
					transform.GetChild(i).GetComponent<BranchManager>().setHasDisease(true);
				}
			}
			else*/ if(transform.GetChild(i).name.Length >= 4) {
				if(transform.GetChild(i).name.Substring(0, 4) == "Leaf") {
					transform.GetChild(i).GetComponent<LeafManager>().setIsAlive(false);
				}
			}
		...
		if(depth > 1)
			transform.parent.GetComponent<BranchManager>().setHasDisease(true);
	}
